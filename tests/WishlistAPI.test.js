...
describe("GET /wishlist", () => {
    it("should respond to the GET method", async () => {
        ...
        expect(response.text).toEqual("Wishlist API, welcome!");
    });
    
    it("should return a 404 status for an invalid route", async () => {
        const response = await request.get("/wishlist/invalid-route");
        expect(response.status).toBe(404);
    });
});

describe("GET /wishlist/product/:productid", () => {
    it("should respond to the GET method", async () => {
        const productid = 1;
        const response = await request.get(`/wishlist/product/${productid}`);
        expect(response.status).toBe(200);
        expect(response.body).toBeDefined();
    });
});
...
describe("GET /wishlist/:id", () => {
    it("should respond to the GET method", async () => {
        const id = 1;
        const response = await request.get(`/wishlist/${id}`);
        expect(response.status).toBe(200);
        expect(response.body).toBeDefined();
    });
});

describe("GET /wishlist/check/:customerid/:productid", () => {
    it("should respond to the GET method", async () => {
        const customerid = 1;
        const productid = 1;
        const response = await request.get(`/wishlist/check/${customerid}/${productid}`);
        expect(response.status).toBe(200);
        expect(response.body).toBeDefined();
    });
});